...
class PruneServerMetricCommand extends Command
{
    ...
    {
        if (!$this->isEnabled()) {
            $this->info('Server metric prune is disabled. If you want to enable it, check the sitevigilance config file.');
    ...
    {
        return config('sitevigilance.prune_server_monitoring.enabled');
    }

    public static function getServerMetricAge(): int
    {
        return config('sitevigilance.prune_server_monitoring.prune_server_monitoring_records_older_than_days');
    }
