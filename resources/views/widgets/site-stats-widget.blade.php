<x-filament-widgets::widget>
    ...
    @endif
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3"{!! ($pollingInterval = $this->getPollingInterval()) ? "wire:poll.{$pollingInterval}" : '' !!}>
        @foreach ($sites as $site)
            <x-filament::card>
                <div class="relative">
                    @if ($site->down_for_maintenance_at)
                        <p class="absolute top-0 right-0 mt-2 text-xs font-bold">
                            On maintenance
                        </p>
                    @endif
                    <div class="divide-y divide-gray-200">
                        ...
                        <div class="pt-2 space-y-2">
                            <x-site-stats-widget.uptime-list-item :site="$site" />
                            <x-site-stats-widget.performace-list-item :site="$site" />
                            <x-site-stats-widget.certificate-list-item :site="$site" />
                            <x-site-stats-widget.exceptions-list-item :site="$site" />
                        </div>
                    </div>
                </div>
            </x-filament::card>
